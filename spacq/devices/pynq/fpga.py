...
class DACPort(AbstractSubdevice):
    ...
    def _connected(self):
        ...
        r = self.device.ask_raw(
            ':5000/dac1?channel={0}&span=pm10V'.format(self.num))
        ...
        r = self.device.ask_raw(
            ':5000/dac1?channel={0}&value=0'.format(self.num))
        # Verify that the command was successfully executed
        if r.status_code != 200:
        ...
        # Set span. Limit is +/- 10, but these values prevent a floating point error realted bug
        self.span_limits = [-9.999999, 9.999999]
    ...
    def voltage(self, value):
        ...
        if value <= self.span_limits[0] or value >= self.span_limits[1]:
            raise ValueError("Voltage {0} is out of the allowed span {1}. (Note: Span excludes end points)".format(
                value, self.span_limits))

        # Verify that the command was successfully executed
        r = self.device.ask_raw(
            ':5000/dac1?channel={0}&value={1}'.format(self.num, value))
        if r.status_code != 200:
...
class ADCPort(AbstractSubdevice):
    ...
    def _connected(self):
        ...
        r = self.device.ask_raw(
            ':5000/adc2?channel={0}&on=true&inputpair=vin{1}vincom&filter=default'.format(self.num, self.num))
    ...
    def filter(self, value):
        ...
        if value not in possible_filters:
            raise ValueError('Filter {0} not part of the allowed powers: {1}'.format(
                value, possible_filters))
...
class OSCPort(AbstractSubdevice):
    ...
    def _connected(self):
        ...
        r = self.device.ask_raw(
            ':5000/osc1?channel={0}&on=true&freq=500e6&power=0&freq_resolution=100e3&power_down=true'.format(self.num))
    ...
    def frequency(self, value):
        ...
        if value < minFreq or value > maxFreq:
            raise ValueError('Value {0} not within the allowed bounds: {1} to {2}'.format(
    ...
    def power(self, value):
        ...
        # corresponds to -5, -2, 1 or 4 dBm, respectively
        possible_powers = ['0', '1', '2', '3']
        if value not in possible_powers:
            raise ValueError('Value {0} not part of the allowed powers: {1}'.format(
                value, possible_powers))
    ...
    def power_on(self, value):
        ...
        if value not in possible_values:
            raise ValueError('Value {0} not part of the allowed values: {1}'.format(
                value, possible_values.values()))

        # Set power_down attribute of oscillator
        r = self.device.ask_raw(
            ':5000/osc1?power_down={0}'.format(possible_values[value]))
