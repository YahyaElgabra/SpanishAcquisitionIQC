...
class Marker(AbstractSubdevice):
    ...
    def delay(self, v):
        self.device.write('source{0}:marker{1}:delay {2}'.format(
            self.channel, self.number, v))
    ...
    def high(self, v):
        self.device.write('source{0}:marker{1}:voltage:high {2}'.format(
            self.channel, self.number, v))
    ...
    @quantity_unwrapped('V')
    def low(self, v):
        self.device.write('source{0}:marker{1}:voltage:low {2}'.format(
            self.channel, self.number, v))
...
class Channel(AbstractSubdevice):
    ...
    def amplitude(self, v):
        ...
        self.device.write(
            'source{0}:voltage {1:E}'.format(self.channel, 2 * v))
...
class AWG5014B(AbstractDevice):
    ...
    def sampling_rate(self, value):
        if value < 1e7 or value > 1.2e9:
            raise ValueError(
                'Sampling rate must be between 1e7 Hz and 1.2e9 Hz, not {0:n} Hz'.format(value))
    ...
    def run_mode(self):
        ...
        mode = self.ask('awgcontrol:rmode?').lower().decode()

        # ask returns bytes, so convert to string
    ...
    def get_waveform(self, name):
        ...
        try:
            log.debug('Getting waveform "{0}" from device "{1}".'.format(
                name, self.name))

            block_data = self.ask_raw(
                'wlist:waveform:data? "{0}"'.format(name))
            ...
            log.debug('Got waveform "{0}" from device "{1}": {2!r}'.format(
                name, self.name, data))
    ...
    def create_waveform(self, name, data, markers=None):
        ...
        try:
            log.debug('Creating waveform "{0}" on device "{1}" with data: {2!r}'.format(
                name, self.name, data))

            min_value, max_value = self.value_range
            range_diff = max_value - min_value
            data = [min_value + int(range_diff * (x + 1.0) / 2.0)
                    for x in data]

            waveform_length = len(data)
            self.write('wlist:waveform:new "{0}", {1}, integer'.format(
                name, waveform_length))
            ...
            if markers:
                ...
                for extra in extra_markers:
                    log.warning('Marker {0} ignored: {1!r}'.format(
                        extra, markers[extra]))
            ...
            self.write('wlist:waveform:data "{0}", {1}'.format(
                name, block_data))
        finally:
            self.status.pop()
    ...
    def enabled(self):
        ...
        # ask returns bytes, so convert to string
        state = self.ask('awgcontrol:rstate?').decode()
