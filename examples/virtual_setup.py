...
class VariableSweepFrame(wx.Frame):
	def __init__(self, parent, *args, **kwargs):
		...
		# Frame
		frame_box = wx.FlexGridSizer(4,1)
		...
		frame_box.Add(self.dependent_panel, 1, flag=wx.EXPAND|wx.ALIGN_CENTER_VERTICAL)
	...
	def OnButton(self, evt=None):
		...
		for i in range(0,self.virtual_panel.var_count.Value):
			...
			virtVars[i] = virtLinSpaceConfig(name, initial, final, steps, order)
		...
		RetrieveValues = [var.DependentFunctionMath(sweepSweep.names,sweepSweep.value_history) for var in realVars]
